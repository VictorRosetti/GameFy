...
function EscondeBotao()
{
  ...
  if(pagina==0 && current_title=='exercicios')
  {
    $('#voltar').hide();
    BuscarPergunta();
    //GetRespostaForm();
    console.log("entrei aqui: "+pagina);
  }else if (pagina<paginaMax && current_title=='exercicios')
  {
    $('#voltar').show();
    BuscarPergunta();
    //GetRespostaForm();
...
$(document).ready(function () {
  alive = localStorage.getItem("Senha");
  let current_title = $(document).attr('title');
  if(alive==0 && current_title!= "login")
  {
    window.location = `http://127.0.0.1:3000/cadastro.html`;
  }
  EscondeBotao();
...
{
  let teste = $("input[name='resposta']:checked").val();
  console.log("Entrei na resposta"+teste);
...
function Login() {
  $.post("http://127.0.0.1:3000/login",
    ...
    , function (msg) {
      ...
      arrayAlive.push(msg);
      localStorage.setItem("Senha",msg);
      if (msg != 0) {
